...
void StaticClass::insert_value_into_field(Value value, const string& fieldName)
{
  static_fields[fieldName] = value;
}

Value StaticClass::get_value_from_field(std::string fieldName)
{
  if (this->static_fields.count(fieldName) == 0) {
    cerr << "NoSuchFieldError" << endl;
    exit(1);
  }

  return this->static_fields[fieldName];
}

bool StaticClass::field_exists(const std::string& fieldName)
{
