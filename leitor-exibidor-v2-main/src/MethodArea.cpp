...
#include "ClassFileReader.hpp" // talvez retirar isso e colocar a getFormattedConstant() na Util.
...
StaticClass* MethodArea::load_class_by_name(const string &className) {
  // se a classe já tiver sido carregada, retorna-la
  if (classes.count(className) > 0) {
    return get_class_by_name(className);
  }
...
StaticClass* MethodArea::get_class_by_name(const string &className) {
